...
Python file to train and evaluate a VGG-16 pretrained model on the Brain Tumor Dataset found on Kaggle. This VGG16 version also performs oversampling across all 3 tumor classes to have an equal amount of all 3 classes in the training dataset. Gliomas are the overrepresented class, so meningiomas and metastatic tumor instances are sampled with replacement to make a new dataset with equal amounts of images for each class.
...
for every tumor class. 
'''

def load_train_data(data_dir):
    
    #Create lists to store the training images and bounding boxes. Make separate lists to store images and bounding boxes
    #that have a class one label (meningiomas). Two other lists are made to store the image paths and bounding boxes that have
    #a class two label (metastatic tumors). Track the amount of images with a class zero label, class one label, and class two
    #label. 
        ...
        if file.endswith('.jpg'):
            ...
            if os.path.isfile(label_path):
                with open(label_path, 'r') as label_file:
                    ...
                    #Append the image path into the class_two_image_paths list if it has a metastatic tumor-labeled bounding box.
                    #Store the coordinates of this bounding box in the class_two_boxes list. Update the total number of labeled
                    #metastatic tumors across all images in the training dataset.
                    
                    if(label==2):
                        class_two_image_paths.append(image_path)
                        class_two_boxes.append((x_one, y_one, x_two, y_two))
                        class_two_count+=1
    ...
    #in specific images, image paths to images with metastatic tumors, bounding boxes that correspond to metastatic tumors, 
    #number of gliomas in the entire dataset, number of meningiomas, and number of metastatic tumors.
    
    return images, boxes, class_one_images, class_one_boxes, class_two_image_paths, class_two_boxes, class_zero_count, class_one_count, class_two_count
...
#class two training image paths, class two bounding boxes, number of images with class zero labels, number of images with class #one labels, and number of images with class two labels into variables that can be used later.

train_images, train_boxes, train_images_class_one, train_boxes_class_one, train_imgpths_class_two, train_boxes_class_two, class_zero_count, class_one_count, class_two_count = load_train_data(train_dataset_dir)


# In[78]:

#For loop to perform oversampling for gliomas: select a random image that has a glioma tumor and add it to the main training images list. Add the bounding box which has the coordinates of this glioma to the main bounding boxes list. Perform this task until the number of glioma labels in the dataset is equal to the number of meningiomas. 
...
#For loop to perform oversampling for metastatic tumors: select a random image that has a metastatic tumor using a random #filepath and add it to the main training images list as a NumPy array. Add the bounding box which has the coordinates of this #metastatic tumor to the main bounding boxes list. Perform this task until the number of metastatic tumor labels in the dataset #is equal to the number of gliomas. 
       
for i in range(class_zero_count-class_two_count):
    index = random.randint(0, class_two_count-1)
    train_images.append(img_to_array(load_img(train_imgpths_class_two[index], target_size=(224, 224))))
    train_boxes.append(train_boxes_class_two[index])
...
#Instantiate the VGG16 model with pretrained imagenet weights, an input_shape that is equal to the resized image size, and no
#3 fully-connected layers near the top of the network.
...
#weights for the training and evaluation of the VGG16 model. Also, it is used to keep the training time manageable because it 
#requires too many computational resources (ex. time, GPUs, CPU cores) otherwise. 
...
model = Model(inputs=vgg16.input, outputs=bounding_box_head)
...
model.compile(loss="mean_squared_error", optimizer=opt, metrics=['accuracy', 'precision', 'recall', 'auc', 'f1_score'])
...
df.to_csv('VGG16Code_Oversampling.csv') 
...
model.save('VGG16Code_Oversampling.keras')
