...
Python file to train and evaluate a MobileNetV3Large pretrained model on the Brain Tumor Dataset found on Kaggle. This MobileNetV3Large version also performs oversampling across all 3 tumor classes to have an equal amount of all 3 classes in the training dataset. Gliomas are the overrepresented class, so meningiomas and metastatic tumor instances are sampled with replacement to make a new dataset with equal amounts of images for each class.
...
df.to_csv('MobileNetV3Large_Oversampling.csv') 
...
model.save('MobileNetV3Large_Oversampling.keras')
